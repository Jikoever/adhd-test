...
1. 在 Supabase 创建项目。
2. 在 Supabase 里确认 Email Auth 已启用（Authentication -> Providers -> Email）。
3. 在 Supabase Auth URL 配置里设置：
   - `Site URL`：`https://<你的线上域名>`
   - `Redirect URLs`：`http://localhost:8080`, `https://<你的线上域名>`
4. 如果你使用 Magic Link，确认邮件模板已开启并能正常发送。
5. 执行数据库迁移（使用本仓库 `supabase/migrations` 下 SQL）。

注意：

- 你启用 Email Auth 的 Supabase 项目，必须和前端 `.env` 里的 `VITE_SUPABASE_URL` / `VITE_SUPABASE_PUBLISHABLE_KEY` 指向的是同一个项目。
- 如果登录邮件发不出去，先检查当前页面实际连接的项目 ref、`Site URL`、`Redirect URLs` 和邮件发送配置是否都在同一个项目里。
